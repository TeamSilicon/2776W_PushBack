...
void default_constants() {
  // P, I, D, and Start I
  chassis.pid_drive_constants_set(19.90, 0.008, 106.75, 1.00);         // Fwd/rev constants, used for odom and non odom motions
  ...
  // Exit conditionsYAYAYAYAYAAYAYAYAYAYAYAYAY HIIIIIII GUYS ICH BIN CODER NOW
...
void burst_burst_right() {
  ...
  // pros::delay(500);
  intake.move(OUTTAKE_SPEED);
  chassis.pid_drive_set(14_in, DRIVE_SPEED + 40);
  chassis.pid_wait();

  matchloaderNutLeft();
  intake.move(0);
  chassis.pid_drive_set(-31_in, DRIVE_SPEED - 20);
  chassis.pid_wait();
  intake.move(OUTTAKE_SPEED);
  outtake.move(OUTTAKE_SPEED);
...
void burst_skills(){
  matchloader.set(true);
  chassis.pid_drive_set(36.75_in, DRIVE_SPEED + 20); // go forward side
  chassis.pid_wait();
  chassis.pid_turn_set(-90_deg, TURN_SPEED); // turn towards matchloader
  chassis.pid_wait();
  // pros::delay(500);
  intake.move(OUTTAKE_SPEED);
  chassis.pid_drive_set(14_in, DRIVE_SPEED + 40); // nut into matchloader
  chassis.pid_wait();
  skillsMatchload();
  intake.move(0);
  chassis.pid_drive_set(-21_in, DRIVE_SPEED - 20); // midddd
  chassis.pid_wait();
  // intake.move(OUTTAKE_SPEED);
  // outtake.move(OUTTAKE_SPEED);
  // pros::delay(OUTTAKE_TIME * 5);
  ...
  // chassis.pid_drive_set(10_in, DRIVE_SPEED);
  // chassis.pid_wait();
  chassis.pid_turn_set(180_deg, TURN_SPEED); // turn to other side
  chassis.pid_wait();
  // other side
  chassis.pid_drive_set(98_in, DRIVE_SPEED + 20); // go to other side
  chassis.pid_wait();
  chassis.pid_turn_set(-90_deg, TURN_SPEED); // face matchaloder
  chassis.pid_wait();
  chassis.pid_drive_set(-10_in, DRIVE_SPEED - 40); // go to goal
  chassis.pid_wait();
  wing.set(true);
  intake.move(OUTTAKE_SPEED);
  outtake.move(OUTTAKE_SPEED);
  pros::delay(OUTTAKE_TIME * 10);
  intake.move(0);
  outtake.move(0); // outtake
  wing.set(false);
  pros::delay(500);
  intake.move(OUTTAKE_SPEED);
  chassis.pid_drive_set(34_in, DRIVE_SPEED + 40); // front
  chassis.pid_wait();
  skillsMatchload();
  intake.move(0);
  chassis.pid_drive_set(-31_in, DRIVE_SPEED - 20); 
  ...
  chassis.pid_wait();
  intake.move(OUTTAKE_SPEED);
  chassis.pid_drive_set(30_in, DRIVE_SPEED + 40);
  chassis.pid_wait();
  intake.move(0);
}
...
void burst_left() {
  ...
  matchloader.set(false);
  chassis.pid_drive_set(8.75_in, DRIVE_SPEED);
  chassis.pid_wait();
  chassis.pid_turn_set(135_deg, TURN_SPEED);
  chassis.pid_wait();
  intake.move(OUTTAKE_SPEED);
...
void burst_left_demid() {
  ...
  matchloaderNutLeft();
  intake.move(0);
  chassis.pid_drive_set(-32_in, DRIVE_SPEED - 20);
  chassis.pid_wait();
  intake.move(OUTTAKE_SPEED);
  outtake.move(OUTTAKE_SPEED);
  pros::delay(OUTTAKE_TIME * 5);
...
void burst_right() { // THIS ONE
  ...
  intake.move(-OUTTAKE_SPEED_BURST);
  pros::delay(OUTTAKE_TIME * 5); // 1nce is 500ms
  intake.move(0);
}
